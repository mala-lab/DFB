...
def sample_estimator(model, num_classes, feature_list, train_loader):
    ...
    for num_batch, (data, target) in enumerate(train_loader):
        ...
        with torch.no_grad():
            data = Variable(data)
            data = data.cuda()
            output, out_features = model(input=data, layer_index='all')
...
def get_Mahalanobis_score(inputs, model, num_classes, sample_mean, precision, num_output, magnitude, domain=False):
    if domain:
        data = Variable(inputs, requires_grad=True)
        data = data.cuda()
        out = model(input=data)
        bg_score = out[:, 0].cpu().detach().numpy()
    for layer_index in range(num_output):

        data = Variable(inputs, requires_grad=True)
        data = data.cuda()

        out_features = model(input=data, layer_index=layer_index)
        ...
        noise_out_features = model(input=Variable(tempInputs), layer_index=layer_index)
    ...
    if domain:
        Mahalanobis_scores = np.concatenate(
            (Mahalanobis_scores, bg_score.reshape((bg_score.shape[0], -1))), axis=1)
