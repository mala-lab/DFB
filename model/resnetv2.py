...
class ResNetV2(nn.Module):
  ...
  def __init__(self, block_units, width_factor, head_size=21843, zero_head=False, dense=False, half_stride=False):
    super().__init__()
    wf = width_factor  # shortcut 'cause we'll use it a lot.
    self.dense = dense
    self.half_stride = half_stride
    ...
    self.body = nn.Sequential(OrderedDict([
        ...
        ('block4', nn.Sequential(OrderedDict(
            [('unit01', PreActBottleneck(cin=1024*wf, cout=2048*wf, cmid=512*wf, stride=1 if self.half_stride else 2))] +
  ...
  def forward(self, input, layer_index=None):
    if layer_index is not None:
        return self.intermediate_forward(input, layer_index)
    if self.dense:
        x = self.body(self.root(input))
        x = self.head[0](x)
        x = self.head[1](x)
        x = self.head[3](x)
        x = F.interpolate(x, size=input.size()[2:], mode='bilinear', align_corners=True)
        return x
    x = self.head(self.body(self.root(input)))
  ...
  def load_state_dict_custom(self, state_dict):
      ...
      for k, v in state_dict.items():
          if k == 'module.head.conv.bias' or k == 'module.head.conv.weight':
              continue
